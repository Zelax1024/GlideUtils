...
import com.zelax.glidelibrary.R;
...
/**
 * Created by Zelax on 2018/7/27.
 */
public class CircleProgressView extends ProgressBar {



    private int mReachBarSize = DensityUtils.dp2px(getContext(), 2); // 未完成进度条大小
    private int mNormalBarSize = DensityUtils.dp2px(getContext(), 2); // 未完成进度条大小
    private int mReachBarColor = Color.parseColor("#108ee9"); // 已完成进度颜色
    private int mNormalBarColor = Color.parseColor("#FFD3D6DA"); // 未完成进度颜色
    private int mTextSize = DensityUtils.sp2px(getContext(), 14); // 进度值字体大小
    ...
    private int mRadius = DensityUtils.dp2px(getContext(), 20); // 半径
    private int mStartArc; // 起始角度
    private int mInnerBackgroundColor; // 内部背景填充颜色
    private int mProgressStyle = ProgressStyle.NORMAL; // 进度风格
    private int mInnerPadding = DensityUtils.dp2px(getContext(), 1); // 内部圆与外部圆间距
    ...
    private int mOuterSize = DensityUtils.dp2px(getContext(), 1); // 外层圆环宽度
    ...
    private void drawFillInArcCircle(Canvas canvas) {
        canvas.save();
        canvas.translate(mRealWidth*1f / 2, mRealHeight*1f  / 2);
        // 绘制外层圆环
    ...
    private void drawNormalCircle(Canvas canvas) {
        ...
        // 绘制内部圆形背景色
        if (needDrawInnerBackground) {
            canvas.drawCircle(0, 0, mRadius - Math.min(mReachBarSize, mNormalBarSize) *1f / 2,
    ...
    public void setReachBarSize(int reachBarSize) {
        mReachBarSize = DensityUtils.dp2px(getContext(), reachBarSize);
        invalidate();
    }
    ...
    public void setNormalBarSize(int normalBarSize) {
        mNormalBarSize = DensityUtils.dp2px(getContext(), normalBarSize);
        invalidate();
    }
    ...
    public void setTextSize(int textSize) {
        mTextSize = DensityUtils.sp2px(getContext(), textSize);
        invalidate();
    }
    ...
    public void setRadius(int radius) {
        mRadius = DensityUtils.dp2px(getContext(), radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = DensityUtils.dp2px(getContext(), innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = DensityUtils.dp2px(getContext(), outerSize);
        invalidate();
    }
